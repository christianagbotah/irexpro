...
## Current Phase: Phase 1 Sprint 19 In Progress — Production Deployment Foundation
...
1. **Production deployment runbook** — `docs/runbooks/production-deployment-vps-webuzo.md`. Covers: system requirements, PostgreSQL setup + hardening, Redis setup + hardening, environment variable setup (with strong-secret generation), build process (NestJS + Python), migration run process (including the Sprint 18 pre-migration duplicate check), PM2 + systemd process management, Nginx reverse proxy + Let's Encrypt TLS, health-check verification, logs + log rotation, rollback process (code + database + configuration), and a full pre-deploy + post-deploy checklist.

2. **Secrets policy runbook** — `docs/runbooks/secrets-never-committed.md`. The authoritative list of files and values that must never be committed to git, with a pre-push self-check script and incident-response steps if a secret is leaked.

3. **PM2 ecosystem example** — `infrastructure/pm2/ecosystem.config.js`. Manages both the NestJS API and the Python AI engine from one config file. Contains NO secrets — all read from gitignored `.env` files.

4. **systemd unit examples** — `infrastructure/systemd/irexpro-api.service` and `infrastructure/systemd/irexpro-ai-engine.service`. Alternative to PM2 for teams that prefer native systemd. Contain NO secrets; hardened with `NoNewPrivileges`, `ProtectSystem`, `ProtectHome`, memory limits.

5. **`.env.example` improvements** — both `apps/api/.env.example` and `services/ai-engine/.env.example` gained production-deployment pointers and per-variable production notes. No real secrets were added; every value remains a `CHANGE_ME_*` / `PLACEHOLDER` / `dev_*` placeholder.

6. **`.gitignore` hardened** — added `*.egg-info/`. Untracked 5 pre-existing `irexpro_ai_engine.egg-info/` files (kept on disk; they are auto-generated by `pip install -e .`).

### Sprint 19 safety rules preserved (no regressions)

\- No production logic, migrations, payment-state transitions, webhook processing, broker, risk, execution, or AI code was changed.
\- Checkout never marks paid — only a verified provider webhook does.
\- HWM updates only through the verified performance-fee webhook success path; cannot regress (Sprint 18 `max()` semantics).
\- Broker credentials remain AES-256-GCM encrypted; never in responses, logs, audit, WebSocket, or errors.
...
\- No demo-data / database-failure fallback. No SQLite. No Fovi-style Next.js API routes or localStorage auth.
\- Production failures fail closed.
\- No real secrets or production credentials added to any committed file, doc, test, or example.

### Verification

\- NestJS tests: 739 passing across 44 suites (unchanged from Sprint 18 — no source logic changed).
\- NestJS build: exit code 0.
\- No new migrations created; no migrations changed.
\- No new libraries installed.

### Relationship to the long-term deployment target

The existing `docs/architecture/15-devops-and-deployment.md` describes the Phase 2 Kubernetes/AWS production target. Sprint 19's VPS/Webuzo runbook is the **first production target** — a simpler single-node deployment that lets iRexPro go live on a single VPS before the full K8s infrastructure is built. The two approaches are complementary; the VPS runbook does not replace or contradict the K8s architecture.
