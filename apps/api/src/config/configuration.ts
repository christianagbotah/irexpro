...
export default () => {
  ...
  const defaultHost = env === 'production' ? '127.0.0.1' : '0.0.0.0';

  return {
    app: {
      port: parseInt(process.env.APP_PORT ?? '3000', 10),
      host: process.env.APP_HOST ?? defaultHost,
    ...
    email: {
      ...
      fromAddress:
        process.env.EMAIL_FROM ?? process.env.EMAIL_FROM_ADDRESS ?? 'no-reply@irexpro.com',
    },
    sms: {
      twilio: {
