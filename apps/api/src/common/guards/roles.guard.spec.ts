...
describe('RolesGuard — admin endpoint enforcement', () => {
  ...
  const mockHandler = function adminHandler() {
    /* noop */
  };
  ...
  beforeEach(() => {
    ...
    jest
      ...
      .mockReturnValue([RoleName.ADMIN, RoleName.SUPER_ADMIN]);
  });

  it('should allow access for a user with ADMIN role', () => {
    const ctx = mockExecutionContext({ id: 'user-1', roles: [RoleName.ADMIN] }, [RoleName.ADMIN]);
    expect(guard.canActivate(ctx)).toBe(true);
  });

  it('should allow access for a user with SUPER_ADMIN role', () => {
    const ctx = mockExecutionContext({ id: 'user-1', roles: [RoleName.SUPER_ADMIN] }, [
  ...
  it('should REJECT a normal USER role (403 Forbidden)', () => {
    const ctx = mockExecutionContext({ id: 'user-2', roles: [RoleName.USER] }, [RoleName.USER]);
    expect(() => guard.canActivate(ctx)).toThrow(ForbiddenException);
    expect(() => guard.canActivate(ctx)).toThrow('Insufficient permissions');
  });

  it('should REJECT a user with no roles array', () => {
    const ctx = mockExecutionContext({ id: 'user-3', roles: [] }, []);
    expect(() => guard.canActivate(ctx)).toThrow(ForbiddenException);
  });
  ...
  it('should allow access when no roles are required (no @Roles decorator)', () => {
    ...
    const ctx = mockExecutionContext({ id: 'user-4', roles: [RoleName.USER] }, [RoleName.USER]);
    expect(guard.canActivate(ctx)).toBe(true);
  });

  it('should allow a user with both USER and ADMIN roles (has ADMIN)', () => {
    const ctx = mockExecutionContext({ id: 'user-5', roles: [RoleName.USER, RoleName.ADMIN] }, [
      RoleName.USER,
      RoleName.ADMIN,
    ]);
    expect(guard.canActivate(ctx)).toBe(true);
