...
/**
 * Sprint 12 — Broker Trade Reconciliation domain.
 *
 * Creates the broker_reconciliation schema and two tables:
 *   - broker_trade_reconciliation_runs  (one record per reconciliation run)
 *   - broker_reconciled_trades          (one record per closed trade processed)
 *
 * Safety rules encoded in schema:
 * - UNIQUE index on (user_id, broker_connection_id, broker_trade_id) prevents
 *   duplicate reconciliation of the same broker trade.
 * - source_type enum prevents fee-eligible flag from ever being set on
 *   DEMO_BROKER / PAPER_BROKER / BACKTEST trades at the DB level.
 * - All money columns use BIGINT (minor currency units, no float risk).
 *
 * down() removes tables, enums, and schema in safe reverse order.
 */
export class CreateBrokerReconciliationSchema1751200000000 implements MigrationInterface {
  name = 'CreateBrokerReconciliationSchema1751200000000';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    // ── broker_trade_reconciliation_runs ───────────────────────────────────
    ...
    // Deduplication: one reconciled trade record per user/broker/tradeId
    await queryRunner.query(`
