"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
var GlobalConfigService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.GlobalConfigService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const typeorm_2 = require("typeorm");
const country_config_entity_1 = require("./entities/country-config.entity");
const country_config_seed_1 = require("./seeds/country-config.seed");
let GlobalConfigService = GlobalConfigService_1 = class GlobalConfigService {
    constructor(countryConfigRepo) {
        this.countryConfigRepo = countryConfigRepo;
        this.logger = new common_1.Logger(GlobalConfigService_1.name);
    }
    async findAllCountries() {
        return this.countryConfigRepo.find({
            where: { isActive: true, isBlocked: false },
            order: { countryName: 'ASC' },
        });
    }
    async findByCountryCode(countryCode) {
        const config = await this.countryConfigRepo.findOne({
            where: { countryCode: countryCode.toUpperCase() },
        });
        if (!config)
            throw new common_1.NotFoundException(`Country configuration not found: ${countryCode}`);
        return config;
    }
    async isCountrySupported(countryCode) {
        const config = await this.countryConfigRepo.findOne({
            where: { countryCode: countryCode.toUpperCase() },
        });
        return !!config && config.isActive && !config.isBlocked;
    }
    async seedCountries() {
        for (const seed of country_config_seed_1.COUNTRY_CONFIG_SEEDS) {
            const existing = await this.countryConfigRepo.findOne({
                where: { countryCode: seed.countryCode },
            });
            if (!existing) {
                await this.countryConfigRepo.save(this.countryConfigRepo.create(seed));
                this.logger.log(`Seeded country: ${seed.countryCode}`);
            }
        }
    }
};
exports.GlobalConfigService = GlobalConfigService;
exports.GlobalConfigService = GlobalConfigService = GlobalConfigService_1 = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(country_config_entity_1.CountryConfig)),
    __metadata("design:paramtypes", [typeorm_2.Repository])
], GlobalConfigService);
//# sourceMappingURL=global-config.service.js.map